...
#define NUM_DATASETS 12
#define NEXT_SQUARE_OF_TWO 16
#define DS_LEN 11
...
__kernel void PartitionData(__global const unsigned short* dataset, __global double* tempVals, __global const unsigned short* splitInfo, 
			__global unsigned short* readSplitInfo, __global double* splitInfoGain, __global unsigned short* categoryBuffer) {
	...
	idx = lid;
	while(idx < NEXT_SQUARE_OF_TWO) {
		readSplitInfo[wid*32+lid] = part1_flags[lid];
		readSplitInfo[wid*32+lid+NEXT_SQUARE_OF_TWO] = part2_flags[lid];
		idx += lsi;
	}
	...
	if(lid == 0) {
		...
		printf("Work-Group %d> InfoGain = (%lf - %lf * %lf - (1 - %lf) * %lf) = %lf\n", wid, tempVals[2], p, curUncertaintyPart1, p, curUncertaintyPart2, infoGain);
